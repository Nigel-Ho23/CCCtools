...
## 5.1. Obtaining unique interaction IDs (CellPhoneDB)

In this example, we use the .txt files generated from our CellPhoneDB analysis (method 2). In our dataset, we have results from two separate experimental conditions.

```{r}

parse_cellphonedb_output <- function(filename, value_name){

out <- read_delim(filename) %>% 
  select(id_cp_interaction, interactions = interacting_pair, partner_a, partner_b, gene_a, gene_b, 
         directionality,classification, contains("|")) %>% 
...
```{r}
# Reading CellPhoneDB output ----------------------------------------------
ctrl.mean <- parse_cellphonedb_output("../../Results/cellphonedb/ctrl/statistical_analysis_means_06_17_2025_115604.txt",   "cpdb_ctrl_mean")
ctrl.pval <- parse_cellphonedb_output("../../Results/cellphonedb/ctrl/statistical_analysis_pvalues_06_17_2025_115604.txt", "cpdb_ctrl_pval")
aga.mean  <- parse_cellphonedb_output("../../Results/cellphonedb/aga/statistical_analysis_means_06_17_2025_143957.txt",    "cpdb_aga_mean")
aga.pval  <- parse_cellphonedb_output("../../Results/cellphonedb/aga/statistical_analysis_pvalues_06_17_2025_143957.txt",  "cpdb_aga_pval")

# Compiling output files from CellPhoneDB (method 2 here); we use the example of two experimental conditions
cpdb.res <- Reduce("full_join", list(ctrl.mean, ctrl.pval, aga.mean, aga.pval)) %>% 
  filter( cpdb_ctrl_pval <= 0.05 | cpdb_aga_pval <= 0.05) %>% 
...
rm(ctrl.mean, ctrl.pval, aga.mean, aga.pval, parse_cellphonedb_output)
```

Mapping example output using the mapped data we curated

```{r}
# Interaction partners to uniprot (CellPhoneDB) ----
complex.map <- read_excel("matched_partial.xlsx", sheet = "clean")
...
cpdb.res <- cpdb.res %>% 
  ...
  left_join(complex.map %>% 
    select(complex_key, CellPhoneDB_name), by = c("partner_a" = "CellPhoneDB_name")) %>% 
  mutate(partner_a = ifelse(!is.na(complex_key), complex_key, partner_a)) %>% 
  select(-complex_key) %>% 
  
  left_join(complex.map %>% 
    select(complex_key, CellPhoneDB_name), by = c("partner_b" = "CellPhoneDB_name")) %>% 
  mutate(partner_b = ifelse(!is.na(complex_key), complex_key, partner_b)) %>% 
  select(-complex_key) %>% 
...
Performing flip of sender and receiver cell types where flip is present

```{r}
# Flipping interactions where necessary
cpdb.res <- cpdb.res %>% 
  left_join(CPDB %>% 
    select(id_cp_interaction, ligand, receptor)) %>% 
  mutate(source = ifelse(partner_a != ligand, cell_b, cell_a),
         target = ifelse(partner_b != receptor, cell_a, cell_b),
...
write_csv(cpdb.res, "results/01.cellphonedb_results.csv")
```

## 5.2. Obtaining unique interaction IDs (CellChat)

Prior to running section 5.2., generate the `Interactions_of_FineCellType_in{condition}.xlsx` file from your CellChat analysis by running:

`df.net_LigandReceptor <- subsetCommunication(cellchat)`

`openxlsx::write.xlsx(df.net_LigandReceptor, file.path(data_path, "Interactions_of_FineCellType_in_{condition.xlsx"))`

```{r}
# Loading CellChat results ------------------------------------------------

ctrl.int <- read_excel("../../Results/cellchat/comparative_analysis/Interactions_of_FineCellType_in_CTRL.xlsx") %>% 
  select(interaction_name, interaction_name_2, pathway_name, everything()) %>% 
  rename(cc_ctrl_prob = prob, cc_ctrl_pval = pval) %>% 
  mutate(unique_int = paste(source, target, interaction_name, sep = "_"))

aga.int <- read_excel("../../Results/cellchat/comparative_analysis/Interactions_of_FineCellType_in_AGA.xlsx") %>% 
  select(interaction_name, interaction_name_2, pathway_name, everything()) %>% 
  rename(cc_aga_prob = prob, cc_aga_pval = pval) %>% 
  mutate(unique_int = paste(source, target, interaction_name, sep = "_"))

aga.only <- anti_join(aga.int, ctrl.int, by = "unique_int")

cc.res <- ctrl.int %>% 
  left_join(aga.int %>% 
    select(unique_int, cc_aga_prob, cc_aga_pval), by = "unique_int") %>% 
  bind_rows(aga.only) %>% 
  select(interaction_name, interaction_name_2, pathway_name, source, target, ligand, receptor, cc_ctrl_prob, cc_ctrl_pval, cc_aga_prob, cc_aga_pval, everything())

cc.res %>% dim() # 23,891

cc.res %>% head()

rm(ctrl.int, aga.int, aga.only)
```

### Creating the unique interaction IDs for CellChat results

```{r}
cc.res <- cc.res %>% 
  left_join(CCDB %>% 
    select(interaction_name, starts_with("uniprot"))) %>% 
  mutate(unique_int = paste(source, target, uniprot_ligand, uniprot_receptor, sep = "_"))

rm(CCDB)

write_csv(cc.res, "results/02.cellchat_results.csv")
```

# 6. Visualise the overlap

In this section, we aim classify interactions based on their respective confidence levels (i.e. Which interactions from CellChat analysis is supported by CellPhoneDB).

We break down the confidence levels as such:

-   High: Interaction is significant in one specified condition only

-   Mid: Interaction is significant in a specified condition, but said interaction is significant in the other condition in CellPhoneDB as well

-   Both sig.: Interaction is significant in both conditions in both CCC tools

-   Low: Interaction is not found or not significant in CellPhoneDB

-   Mismatch: Interaction is significant in one CCC tool but not in the other

```{r}
cc.res <- cc.res %>% 
  left_join(cpdb.res %>% 
    select(unique_int, contains("pval"))) %>% 
  mutate(confidence = case_when(
  cc_ctrl_pval <= 0.05 & cpdb_ctrl_pval <= 0.05 & is.na(cc_aga_pval) & cpdb_aga_pval > 0.05 ~ "High CTRL", # CTRL significant for
                                                                                                           # both
  
  cc_aga_pval <= 0.05 & cpdb_aga_pval <= 0.05 & is.na(cc_ctrl_pval) & cpdb_ctrl_pval > 0.05 ~ "High AGA",  # AGA significant for
                                                                                                           # both
  
  cc_ctrl_pval <= 0.05 & cpdb_ctrl_pval <= 0.05 & is.na(cc_aga_pval) ~ "Mid CTRL", # CTRL significant in both, though significant
                                                                                   # also in AGA
  
  cc_aga_pval <= 0.05 & cpdb_aga_pval <= 0.05 & is.na(cc_ctrl_pval)  ~ "Mid AGA",  # AGA significant in both, though significant
                                                                                   # also in CTRL
  
  cc_ctrl_pval <= 0.05 & cpdb_ctrl_pval <= 0.05 & cc_aga_pval <= 0.05 & cpdb_aga_pval <= 0.05 ~ "Both sig.",
  
  is.na(cpdb_ctrl_pval | cpdb_aga_pval) ~ "Low", # not supported by CellPhoneDB / not significant in CellPhoneDB
...
Visualising overlap at various confidence levels

```{r}
#| fig-width: 24
#| fig-height: 14
plot_data <- cc.res %>%
  group_by(confidence, pathway_name) %>%
  summarise(n = n()) %>% 
  mutate(confidence = factor(confidence, levels = c("High CTRL", "High AGA", "Mid CTRL", "Mid AGA", 
                                                    "Both sig.", "Low",  "Mismatch")),
         pathway_name = reorder_within(pathway_name, n, confidence))

ggplot(plot_data, aes(x = pathway_name, y = n, fill = confidence)) + 
  geom_bar(stat = "identity") +
  ggtitle("Significant pathways in overlap analysis") +
  labs(x = "Pathways",
       y = "Number of interactions per pathway") +
  coord_flip() +
  facet_wrap( ~ confidence, scale = "free", ncol = 7) +
  scale_x_reordered() + # cleans up labels
